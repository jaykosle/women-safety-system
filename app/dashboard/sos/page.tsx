// app/dashboard/sos/page.tsx
'use client'

import { useState, useEffect } from 'react'
import { Plus, Trash2, Phone, Mail, AlertTriangle, CheckCircle, MapPin } from 'lucide-react'

interface Contact { id: string; name: string; phone: string; email?: string }
interface SosAlert {
  id: string; status: string; latitude: number; longitude: number
...
export default function SosPage() {
  ...
  useEffect(() => { fetchContacts(); fetchHistory() }, [])
  ...
  async function triggerSOS() {
    setSosLoading(true); setSosSuccess(false); setError(''); setLocating(true)
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLocating(false); setSosLoading(false)
    ...
    <div className="px-6 py-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl p-8 border border-slate-100 text-center">
        ...
        <button
          ...
          className="w-full max-w-xs mx-auto block py-4 rounded-2xl text-white font-bold text-lg transition-all disabled:cursor-not-allowed"
          style={{
            background: sosLoading || contacts.length === 0
              ? '#fca5a5'
              : 'linear-gradient(135deg, #ef4444, #dc2626)',
            boxShadow: sosLoading || contacts.length === 0
              ? 'none'
        ...
        {contacts.length === 0 && (
          <p className="text-xs text-amber-600 mt-3">
            Add at least one trusted contact below to enable SOS.
          </p>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-100">
        ...
        <form onSubmit={addContact} className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
          <input
            ...
            className="border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-rose-300"
          />
          <input
            required
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-rose-300"
            />
            <button
              type="submit"
              disabled={addLoading}
              className="bg-rose-600 hover:bg-rose-700 text-white px-4 rounded-xl transition-colors disabled:opacity-50"
              title="Add contact"
            >
